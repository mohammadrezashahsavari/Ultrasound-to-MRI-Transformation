...
from Models.CycleGAN.models import modelGenerator, modelDiscriminator
...
class CycleGAN():
    ...
    def train_step(self, real_A, real_B):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            cycle_A = tf.cast(cycle_A, tf.float32)
            real_B = tf.cast(real_B, tf.float32)
            cycle_B = tf.cast(cycle_B, tf.float32)

            #print(real_A.dtype, cycle_A.dtype)
            #exit(0)
