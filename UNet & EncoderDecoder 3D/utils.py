...
'''
def load_dataset(dataset_path, image_names, resize_to):
    images_list = []
    print('Loading ' + dataset_path.split(os.sep)[-1] + ' ...')
    for image_name in tqdm(image_names):
        image_path = os.path.join(dataset_path, image_name)
        nii_img  = nib.load(image_path)
        image = nii_img.get_fdata()
        image = min_max_normalize(image)
        image = resize_volume(image, resize_to)
        image = np.expand_dims(image, axis=-1)
        images_list.append(image)

    images_metadata = {'affine':nii_img.affine, 'header':nii_img.header}
    return np.array(images_list), images_metadata
    '''
